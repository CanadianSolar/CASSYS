...
Function ParsePANFile(ByVal PANFilePath As String) As Boolean
    ...
    If (StrComp(firstLine, "PVObject_=pvModule") = 0) Then
        ParsePANFile = ParseTextPANFile(PANFilePath)
    Else
        ParsePANFile = ParseBinaryPANFile(PANFilePath)
    End If
...
Function ParseTextPANFile(ByVal PANFilePath As String) As Boolean
    ...
    PV_DatabaseSht.Cells(lastRow, 1).Value = "User_Added" & PvModule("Version")
    PV_DatabaseSht.Cells(lastRow, 2).Value = PvCommercial("Manufacturer")
    PV_DatabaseSht.Cells(lastRow, 3).Value = PvCommercial("Model")
    PV_DatabaseSht.Cells(lastRow, 4).Value = Right(PANFilePath, Len(PANFilePath) - InStrRev(PANFilePath, "\"))
    PV_DatabaseSht.Cells(lastRow, 5).Value = PvCommercial("DataSource")
    PV_DatabaseSht.Cells(lastRow, 6).Value = PvCommercial("YearBeg")
    PV_DatabaseSht.Cells(lastRow, 7).Value = PvCommercial("YearEnd")
    PV_DatabaseSht.Cells(lastRow, 8).Value = Technol("PNom")
    PV_DatabaseSht.Cells(lastRow, 10).Value = Technol("Technol")
    PV_DatabaseSht.Cells(lastRow, 11).Value = Technol("NCelS")
    PV_DatabaseSht.Cells(lastRow, 12).Value = Technol("NCelP")
    PV_DatabaseSht.Cells(lastRow, 13).Value = Technol("GRef")
    PV_DatabaseSht.Cells(lastRow, 14).Value = Technol("TRef")
    PV_DatabaseSht.Cells(lastRow, 15).Value = Technol("Vmp")
    PV_DatabaseSht.Cells(lastRow, 16).Value = Technol("Imp")
    PV_DatabaseSht.Cells(lastRow, 17).Value = Technol("Voc")
    PV_DatabaseSht.Cells(lastRow, 18).Value = Technol("Isc")
    PV_DatabaseSht.Cells(lastRow, 19).Value = Technol("muISC")
    PV_DatabaseSht.Cells(lastRow, 21).Value = Technol("muPmpReq")
    PV_DatabaseSht.Cells(lastRow, 22).Value = Technol("Rp_0")
    PV_DatabaseSht.Cells(lastRow, 23).Value = Technol("Rp_Exp")
    PV_DatabaseSht.Cells(lastRow, 24).Value = Technol("RShunt")
    PV_DatabaseSht.Cells(lastRow, 25).Value = Technol("RSerie")
    PV_DatabaseSht.Cells(lastRow, 27).Value = Technol("VMaxIEC")
    PV_DatabaseSht.Cells(lastRow, 28).Value = Technol("NDiode")
    PV_DatabaseSht.Cells(lastRow, 29).Value = Technol("VRevDiode")
    PV_DatabaseSht.Cells(lastRow, 30).Value = Technol("BRev")
    PV_DatabaseSht.Cells(lastRow, 31).Value = CDbl(PvCommercial("Height")) * 1000
    PV_DatabaseSht.Cells(lastRow, 32).Value = CDbl(PvCommercial("Width")) * 1000
    PV_DatabaseSht.Cells(lastRow, 33).Value = CDbl(PvCommercial("Depth")) * 1000
    PV_DatabaseSht.Cells(lastRow, 34).Value = PvCommercial("Weight")
    PV_DatabaseSht.Cells(lastRow, 35).Value = CDbl(PvCommercial("Width")) * CDbl(PvCommercial("Height"))
    PV_DatabaseSht.Cells(lastRow, 38).Value = Technol("CellArea")
    PV_DatabaseSht.Cells(lastRow, 39).Value = CInt(Technol("NCelS")) * CInt(Technol("NCelP")) * CDbl(Technol("CellArea")) / 10000
    PV_DatabaseSht.Cells(lastRow, 44).Value = PvCommercial("PriceDate")
    PV_DatabaseSht.Cells(lastRow, 46).Value = Right(PvCommercial("Str_1"), Len(PvCommercial("Str_1")) - 7)
    PV_DatabaseSht.Cells(lastRow, 47).Value = Right(PvCommercial("Str_2"), Len(PvCommercial("Str_2")) - 11)
    PV_DatabaseSht.Cells(lastRow, 48).Value = PvCommercial("Comment")
    
    If IAMDefined Then
        IAMPoint = Split(PVIAM("Point_1"), ",")
        PV_DatabaseSht.Cells(lastRow, 64).Value = IAMPoint(0)
        PV_DatabaseSht.Cells(lastRow, 65).Value = IAMPoint(1)
        
        IAMPoint = Split(PVIAM("Point_2"), ",")
        PV_DatabaseSht.Cells(lastRow, 66).Value = IAMPoint(0)
        PV_DatabaseSht.Cells(lastRow, 67).Value = IAMPoint(1)
        
        IAMPoint = Split(PVIAM("Point_3"), ",")
        PV_DatabaseSht.Cells(lastRow, 68).Value = IAMPoint(0)
        PV_DatabaseSht.Cells(lastRow, 69).Value = IAMPoint(1)
        
        IAMPoint = Split(PVIAM("Point_4"), ",")
        PV_DatabaseSht.Cells(lastRow, 70).Value = IAMPoint(0)
        PV_DatabaseSht.Cells(lastRow, 71).Value = IAMPoint(1)
        
        IAMPoint = Split(PVIAM("Point_5"), ",")
        PV_DatabaseSht.Cells(lastRow, 72).Value = IAMPoint(0)
        PV_DatabaseSht.Cells(lastRow, 73).Value = IAMPoint(1)
        
        IAMPoint = Split(PVIAM("Point_6"), ",")
        PV_DatabaseSht.Cells(lastRow, 74).Value = IAMPoint(0)
        PV_DatabaseSht.Cells(lastRow, 75).Value = IAMPoint(1)
        
        IAMPoint = Split(PVIAM("Point_7"), ",")
        PV_DatabaseSht.Cells(lastRow, 76).Value = IAMPoint(0)
        PV_DatabaseSht.Cells(lastRow, 77).Value = IAMPoint(1)
        
        IAMPoint = Split(PVIAM("Point_8"), ",")
        PV_DatabaseSht.Cells(lastRow, 78).Value = IAMPoint(0)
        PV_DatabaseSht.Cells(lastRow, 79).Value = IAMPoint(1)
        
        IAMPoint = Split(PVIAM("Point_9"), ",")
        PV_DatabaseSht.Cells(lastRow, 80).Value = IAMPoint(0)
        PV_DatabaseSht.Cells(lastRow, 81).Value = IAMPoint(1)
...
Function ParseBinaryPANFile(ByVal PANFilePath As String) As Boolean
    ...
    PV_DatabaseSht.Cells(lastRow, 4).Value = Right(PANFilePath, Len(PANFilePath) - InStrRev(PANFilePath, "\"))
    PV_DatabaseSht.Cells(lastRow, 2).Value = Left(Manufacturer, Len(Manufacturer) - 1) 'Left() is required to trim the special whitespace character at the end
    PV_DatabaseSht.Cells(lastRow, 3).Value = Left(Model, Len(Model) - 1)
    PV_DatabaseSht.Cells(lastRow, 5).Value = Source
    PV_DatabaseSht.Cells(lastRow, 1).Value = version
    PV_DatabaseSht.Cells(lastRow, 6).Value = inYear
    PV_DatabaseSht.Cells(lastRow, 10).Value = technology
    PV_DatabaseSht.Cells(lastRow, 11).Value = CellsinS
    PV_DatabaseSht.Cells(lastRow, 12).Value = CellsinP
    PV_DatabaseSht.Cells(lastRow, 28).Value = byPassDiodes
    PV_DatabaseSht.Cells(lastRow, 8).Value = PNom
    PV_DatabaseSht.Cells(lastRow, 13).Value = Gref
    PV_DatabaseSht.Cells(lastRow, 9).Value = Toleranz
    PV_DatabaseSht.Cells(lastRow, 35).Value = AreaM
    PV_DatabaseSht.Cells(lastRow, 38).Value = CellArea
    PV_DatabaseSht.Cells(lastRow, 27).Value = vMax
    PV_DatabaseSht.Cells(lastRow, 14).Value = Tref
    PV_DatabaseSht.Cells(lastRow, 18).Value = Isc
    PV_DatabaseSht.Cells(lastRow, 19).Value = mIsc
    PV_DatabaseSht.Cells(lastRow, 17).Value = Voc
    PV_DatabaseSht.Cells(lastRow, 20).Value = mVco
    PV_DatabaseSht.Cells(lastRow, 16).Value = Impp
    PV_DatabaseSht.Cells(lastRow, 15).Value = Vmpp
    PV_DatabaseSht.Cells(lastRow, 29).Value = BypassDiodeVoltage
    PV_DatabaseSht.Cells(lastRow, 24).Value = Rshunt
    PV_DatabaseSht.Cells(lastRow, 25).Value = Rseries
    PV_DatabaseSht.Cells(lastRow, 22).Value = Rsh0
    PV_DatabaseSht.Cells(lastRow, 23).Value = Rshexp
    PV_DatabaseSht.Cells(lastRow, 21).Value = mPmpp
    PV_DatabaseSht.Cells(lastRow, 1).EntireRow.RowHeight = 15
    
    If checkValidVersion(version) = False And dotMarkerCounter = 2 Then
        For i = 0 To 8
            PV_DatabaseSht.Cells(lastRow, 64 + 2 * i).Value = aoi(i)
            PV_DatabaseSht.Cells(lastRow, 64 + 2 * i + 1).Value = Modifier(i)
        Next i
    End If
    
    Call PostModify(PV_DatabaseSht, currentShtStatus)
    
    ' Return true for a successful parse
    ParseBinaryPANFile = True
    Exit Function

    ' Return false for an unsuccessful parse
invalidPANFile:
    On Error GoTo 0
    ParseBinaryPANFile = False
...
Function ParseONDFile(ByVal ONDFilePath As String) As Boolean
    ...
    If (StrComp(firstLine, "PVObject_=pvGInverter") = 0) Then
        ParseONDFile = ParseTextONDFile(ONDFilePath)
    Else
        ParseONDFile = ParseBinaryONDFile(ONDFilePath)
    End If
...
Function ParseTextONDFile(ByVal ONDFilePath As String) As Boolean
    Dim textLine As String
    Dim lineArray() As String
    Dim currentDict As String
    Dim lastRow As Integer
    ...
    lastRow = Inverter_DatabaseSht.Range("A" & Rows.count).End(xlUp).row + 1
    
    Inverter_DatabaseSht.Cells(lastRow, 1).Value = "User_Added " & PvGInverter("Version")
    Inverter_DatabaseSht.Cells(lastRow, 2).Value = PvCommercial("Manufacturer")
    Inverter_DatabaseSht.Cells(lastRow, 3).Value = PvCommercial("Model")
    Inverter_DatabaseSht.Cells(lastRow, 4).Value = Right(ONDFilePath, Len(ONDFilePath) - InStrRev(ONDFilePath, "\"))
    Inverter_DatabaseSht.Cells(lastRow, 5).Value = PvCommercial("DataSource")
    Inverter_DatabaseSht.Cells(lastRow, 6).Value = PvCommercial("YearBeg")
    Inverter_DatabaseSht.Cells(lastRow, 7).Value = PvCommercial("YearEnd")
    Inverter_DatabaseSht.Cells(lastRow, 8).Value = Converter("PNomConv")
    Inverter_DatabaseSht.Cells(lastRow, 9).Value = Converter("PMaxOUT")
    Inverter_DatabaseSht.Cells(lastRow, 10).Value = Converter("INomAC")
    Inverter_DatabaseSht.Cells(lastRow, 11).Value = Converter("IMaxAC")
    Inverter_DatabaseSht.Cells(lastRow, 12).Value = Converter("VOutConv")
    Inverter_DatabaseSht.Cells(lastRow, 13).Value = Converter("MonoTri")
    Inverter_DatabaseSht.Cells(lastRow, 15).Value = Converter("PNomDC")
    Inverter_DatabaseSht.Cells(lastRow, 16).Value = Converter("PMaxDC")
    Inverter_DatabaseSht.Cells(lastRow, 18).Value = Converter("PSeuil")
    Inverter_DatabaseSht.Cells(lastRow, 19).Value = Converter("VmppNom")
    Inverter_DatabaseSht.Cells(lastRow, 20).Value = Converter("VMppMin")
    Inverter_DatabaseSht.Cells(lastRow, 21).Value = Converter("VMPPMax")
    Inverter_DatabaseSht.Cells(lastRow, 22).Value = Converter("VAbsMax")
    Inverter_DatabaseSht.Cells(lastRow, 24).Value = Converter("INomDC")
    Inverter_DatabaseSht.Cells(lastRow, 25).Value = Converter("IMaxDC")
    Inverter_DatabaseSht.Cells(lastRow, 27).Value = Converter("ModeOper")
    Inverter_DatabaseSht.Cells(lastRow, 28).Value = Converter("CompPMax")
    Inverter_DatabaseSht.Cells(lastRow, 29).Value = Converter("CompVMax")
    Inverter_DatabaseSht.Cells(lastRow, 33).Value = Converter("MasterSlave")
    Inverter_DatabaseSht.Cells(lastRow, 34).Value = Converter("NbInputs")
    Inverter_DatabaseSht.Cells(lastRow, 42).Value = Converter("EfficMax")
    Inverter_DatabaseSht.Cells(lastRow, 43).Value = Converter("EfficEuro")
    Inverter_DatabaseSht.Cells(lastRow, 44).Value = Converter("FResNorm")
    
    If (SingleEffCurve) Then
        ' Single Efficiency curve data
        Call AssignEffPoints(ProfilPIO, 2, 8, 45, lastRow)
        
    Else
        ' 3 efficiency curves definition
        Inverter_DatabaseSht.Cells(lastRow, 40).Value = "X"
        ' Assigning Input voltages
        EffPoint = Split(Converter("VNomEff"), ",")
        Inverter_DatabaseSht.Cells(lastRow, 76).Value = EffPoint(0)  ' Low
        Inverter_DatabaseSht.Cells(lastRow, 93).Value = EffPoint(1)  ' Med
        Inverter_DatabaseSht.Cells(lastRow, 110).Value = EffPoint(2) ' High
        ' Assigning Max Efficiency Voltage
        EffPoint = Split(Converter("EfficMaxV"), ",")
        Inverter_DatabaseSht.Cells(lastRow, 77).Value = EffPoint(0)  ' Low
        Inverter_DatabaseSht.Cells(lastRow, 94).Value = EffPoint(1)  ' Med
        Inverter_DatabaseSht.Cells(lastRow, 111).Value = EffPoint(2) ' High
        ' Assigning Max Euro Efficiency Voltage
        EffPoint = Split(Converter("EfficEuroV"), ",")
        Inverter_DatabaseSht.Cells(lastRow, 78).Value = EffPoint(0)  ' Low
        Inverter_DatabaseSht.Cells(lastRow, 95).Value = EffPoint(1)  ' Med
        Inverter_DatabaseSht.Cells(lastRow, 112).Value = EffPoint(2) ' High
        ' Assigning points for 3 efficiency curves
        Call AssignEffPoints(ProfilPIOV1, 2, 8, 79, lastRow)
        Call AssignEffPoints(ProfilPIOV2, 2, 8, 96, lastRow)
        Call AssignEffPoints(ProfilPIOV3, 2, 8, 113, lastRow)
    End If
    
    ' The below set of values are not important to the simulation so skip them if they cannot be read
    On Error Resume Next
    
    Inverter_DatabaseSht.Cells(lastRow, 59).Value = PvCommercial("Height")
    Inverter_DatabaseSht.Cells(lastRow, 60).Value = PvCommercial("Width")
    Inverter_DatabaseSht.Cells(lastRow, 61).Value = PvCommercial("Depth")
    Inverter_DatabaseSht.Cells(lastRow, 62).Value = PvCommercial("Weight")
    Inverter_DatabaseSht.Cells(lastRow, 63).Value = PvCommercial("Currency")
    Inverter_DatabaseSht.Cells(lastRow, 68).Value = PvCommercial("NPieces")
    Inverter_DatabaseSht.Cells(lastRow, 70).Value = Right(PvCommercial("Str_1"), Len(PvCommercial("Str_1")) - 11)
    ...
    If (StrComp(PvCommercial("Str_4"), "Bipolar inputs") = 0) Then
        Inverter_DatabaseSht.Cells(lastRow, 72).Value = "TRUE"
    Else
        Inverter_DatabaseSht.Cells(lastRow, 72).Value = "FALSE"
    End If
    
    Inverter_DatabaseSht.Cells(lastRow, 73).Value = PvCommercial("Comment")
    
    Call PostModify(Inverter_DatabaseSht, currentShtStatus)
    
    ' Reset dictionaries
    Set PvGInverter = Nothing
    Set PvCommercial = Nothing
    Set Converter = Nothing
    Set ProfilPIO = Nothing
    Set ProfilPIOV1 = Nothing
    Set ProfilPIOV2 = Nothing
    Set ProfilPIOV3 = Nothing
    
    ParseTextONDFile = True
    Exit Function
...
Function ParseBinaryONDFile(ByVal ONDFilePath As String) As Boolean
   
   
    Dim byteArray() As Byte ' The byte array used to store all the bytes in the file; each Index represents one byte
    Dim lastRow As Integer ' Denotes the last row in the inverter database where the new inverter should be written
    ...
    ' Extract the PAN file name from the full path
    Inverter_DatabaseSht.Cells(lastRow, 4).Value = Right(ONDFilePath, Len(ONDFilePath) - InStrRev(ONDFilePath, "\"))
    Inverter_DatabaseSht.Cells(lastRow, 2).Value = Left(Manufacturer, Len(Manufacturer) - 1) 'Left() is required to trim the special whitespace character at the end
    Inverter_DatabaseSht.Cells(lastRow, 3).Value = Left(Model, Len(Model) - 1)
    Inverter_DatabaseSht.Cells(lastRow, 5).Value = Source
    Inverter_DatabaseSht.Cells(lastRow, 1).Value = version
    Inverter_DatabaseSht.Cells(lastRow, 6).Value = inYear
    Inverter_DatabaseSht.Cells(lastRow, 27).Value = Operation
    Inverter_DatabaseSht.Cells(lastRow, 13).Value = phaseType
    Inverter_DatabaseSht.Cells(lastRow, 8).Value = PNomAC
    Inverter_DatabaseSht.Cells(lastRow, 22).Value = MaxVoltage
    Inverter_DatabaseSht.Cells(lastRow, 23).Value = MinV
    Inverter_DatabaseSht.Cells(lastRow, 12).Value = Output
    Inverter_DatabaseSht.Cells(lastRow, 20).Value = MinMPP
    Inverter_DatabaseSht.Cells(lastRow, 21).Value = MaxMPP
    Inverter_DatabaseSht.Cells(lastRow, 18).Value = Threshold
    Inverter_DatabaseSht.Cells(lastRow, 9).Value = PMaxAC
    Inverter_DatabaseSht.Cells(lastRow, 24).Value = NomCurrentDC
    Inverter_DatabaseSht.Cells(lastRow, 19).Value = NomVoltageDC
    Inverter_DatabaseSht.Cells(lastRow, 15).Value = PNomDC
    Inverter_DatabaseSht.Cells(lastRow, 16).Value = PMaxDC
    Inverter_DatabaseSht.Cells(lastRow, 25).Value = MaxCurrent
    Inverter_DatabaseSht.Cells(lastRow, 10).Value = InomAC
    Inverter_DatabaseSht.Cells(lastRow, 11).Value = IMaxAC
    Inverter_DatabaseSht.Cells(lastRow, 44).Value = Res
    Inverter_DatabaseSht.Cells(lastRow, 72).Value = bipolarInputs
    Inverter_DatabaseSht.Cells(lastRow, 1).EntireRow.RowHeight = 15
    
    
    Call WriteEffCurveToDatabase(lastRow, databaseStandardEffStart, standardEffCurve)
    
    ' If the inverter is multi-curved then write the low, med and high curves to the database as well
    If isMultiCurved Then
        Inverter_DatabaseSht.Cells(lastRow, 40).Value = "X"
        Call WriteEffCurveToDatabase(lastRow, databaseLowEffStart, lowEffCurve)
        Call WriteEffCurveToDatabase(lastRow, databaseMedEffStart, medEffCurve)
        Call WriteEffCurveToDatabase(lastRow, databaseHighEffStart, highEffCurve)
    End If
    
    ' Clear all invalid values (-9999)
    For Each aCell In Inverter_DatabaseSht.Range("A" & lastRow, "Y" & lastRow)
        If aCell.Value = -9999 Then aCell.Value = vbNullString
    Next

    Call PostModify(Inverter_DatabaseSht, currentShtStatus)
    
    ' Return a true for a successful parse
    ParseBinaryONDFile = True
    Exit Function
    
' The code only reaches here if an error due to formatting was encountered
invalidONDFile:
    On Error GoTo 0
    ParseBinaryONDFile = False
